...
with open(csvpath, newline='') as csvfile:
    ...
    maxIncrease_position=change.index(max(change))
    #print(maxIncrease_position) #to check if position is retreived
    minIncrease_position=change.index(min(change))
...
with open(csvpath, newline='') as csvfile:
    ...
    #dates_change=dict(zip(dates, change))
    #print(dates_change) # to check accuracy of the dict data = missmatch due to no value for the first mo of the change
